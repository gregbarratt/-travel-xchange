...
create table if not exists public.companies (
  ...
  cover_image_fit text not null default 'cover' check (cover_image_fit in ('cover', 'contain')),
  cover_image_position text not null default '50% 50%' check (cover_image_position ~ '^(100|[1-9]?[0-9])% (100|[1-9]?[0-9])%$'),
  description text,
