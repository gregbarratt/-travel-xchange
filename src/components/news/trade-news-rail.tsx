  ...
  return (
    <article className="rounded-lg border border-[var(--tx-border)] bg-white p-5 shadow-[0_10px_22px_rgba(7,36,91,0.06)]">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-extrabold text-[var(--tx-text)]">{heading}</h2>
      </div>

      {isLoading ? (
        <p className="mt-3 text-sm leading-6 text-[var(--tx-text-subtle)]">Loading trade news...</p>
      ) : null}
      ...
      {posts.length > 0 ? (
        <ul className="mt-3 space-y-3">
          {posts.map((post) => (
            <li key={post.id}>
              <a
                className="block rounded-md border border-[var(--tx-border)] p-3 hover:bg-[var(--tx-surface-hover)]"
                href={post.canonical_url}
                rel="noopener noreferrer nofollow"
                target="_blank"
              >
                <p className="text-sm font-bold leading-5 text-[var(--tx-text)]">{post.title}</p>
                <p className="mt-1 inline-flex items-center gap-1 text-xs font-semibold text-[var(--tx-text-subtle)]">
      ...
      <Link
        className="mt-4 inline-flex text-sm font-bold text-[var(--tx-accent)] hover:text-[var(--tx-accent-hover)]"
        href="/news/latest"
      >
        View all trade news
      </Link>
